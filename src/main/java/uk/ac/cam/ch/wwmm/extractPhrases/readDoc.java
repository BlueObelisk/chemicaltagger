	...
	private int checkAppendNextItem(int sectionIndex, List<String> sections) {
		...
		while (newSectionIndex + 1 < sections.size()){
			if (sections.get(newSectionIndex+1).startsWith("Compound")){
				fileInput = fileInput + "EXPERIMENTALDATA="+sections.get(newSectionIndex+1).replace(DELIMITER, " ")+"\n";
			}
			else {
				break;
			}
	...
	private List<String> getSections_Harter(String[] paragraphs) {
		int sectionCount = 0;
		  ...
		  for( int i=0; i<paragraphs.length; i++ ) {
                  ...
                  if (paragraphs[i].length() >20 &&  ! paragraphs[i].startsWith("Experimental") && ! paragraphs[i].startsWith("General experimental")){
                      
             		 int index = paragraphs[i].indexOf("max");
             		 if (index > 1){
             			 sectionString= sectionString+paragraphs[i].substring(0,index)+DELIMITER+paragraphs[i].substring(index,paragraphs[i].length());	 
             		 }
             		 else{
             			 sectionString= sectionString+paragraphs[i]+DELIMITER;	 
             		 }
             		
                	 if (paragraphs[i].length() <100 || paragraphs[i].startsWith("Preparation") || paragraphs[i].startsWith("General")){
